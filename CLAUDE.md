...
**Current Task 17 host evidence (2026-09-01):** frontend tests are 768/768, the desktop Rust library is 1,161/1,161, the MCP-only binary is 941/941, and the exact ARM64 Kotlin contract suite is 11/11. The Android Rust target check/build, clean Gradle APK assembly, APK v2 signature, and packaged-native hash match all pass. Ubuntu CI invokes the generated wrapper through `bash ./gradlew`, so a Windows checkout cannot silently record a non-executable wrapper and break the Linux job. The canonical Tauri wrapper reaches its final native-library handoff but cannot create the Windows symlink while Developer Mode is off; `HOW_TO_RUN.md` records the reviewed copy-and-clean-assemble fallback. The resulting debug APK is 392,359,999 bytes with SHA-256 `F78B05C99C3053CE52D7B8FCBEB88B0831648094A7B5F781AC72AC65CABA56C2`; its 385,088,544-byte packaged native library exactly matches the source SHA-256 `D184078D729A921A916A6E362272BE96D43FE5199947E4A212A7CB7585691499`. No `adb` device is attached, so installed Android runtime behavior is not yet verified.
...
On Windows/MSVC, `build.rs` passes the Common Controls v6 manifest dependency to Rust test harnesses. Tauri's generated resource already owns the desktop application manifest, so the `grafyn` binary uses `/MANIFEST:NO` to avoid a duplicate while the test executables retain the activation dependency. This prevents the `TaskDialogIndirect` loader failure; if a stale `target/debug/deps/grafyn_lib-*.exe` still shows that popup, rebuild the harness rather than launching the stale artifact. The Rust suite is filesystem-heavy, so use `cargo test -- --test-threads=4` on Windows to avoid transient scanner/ACL pressure from unrestricted parallel temp-root churn.

## Architecture Overview

**Current implementation:** Grafyn uses one Tauri 2 Rust library entry point with separate desktop and Android command registrations. Desktop keeps the existing wide shell, user-selected vault, spatial/linear Canvas, Ollama/MCP/import/migration/optimizer/updater surfaces, and one-shot image generation. Android mounts the compact companion shell over app-private storage and exposes the bounded local Capture, Recall, Twin review/chat, chronological one-model Canvas, and native image-share boundary. Backend `RuntimeStatusV1`, loaded before router mount, is authoritative: the frontend's candidate profile can only remove capabilities, never invent them. Android image generation remains false; native Share is a separately health-gated operation over an already validated generated-image receipt. No web mode, Python backend, hosted relay, billing, account, or pairing/recovery service exists.
