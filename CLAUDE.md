...
| **System-test runtime** | Feature-gated Tauri test state + loopback HTTP | `frontend/src-tauri/src/bin/grafyn-test-runtime.rs`; never a release capability | 18890 (tests only) |
...
Desktop environment fallback only: `set OPENROUTER_API_KEY=your-key` (Windows) or `export OPENROUTER_API_KEY=your-key` for Canvas text and one-shot image generation. Android never reads `OPENROUTER_API_KEY`; it uses the native Keystore-backed secret adapter or reports secret-dependent capabilities unavailable.
...
Required toolchain: JDK 17, Android SDK platform `android-36`, Android Build Tools `35.0.0`, Android NDK `28.2.13676358`, and Rust target `aarch64-linux-android`.
...
Set `JAVA_HOME`, `ANDROID_HOME` (or `ANDROID_SDK_ROOT`), and `NDK_HOME` to those installed toolchains before invoking Tauri. This is the canonical debug APK command; do not describe the APK as verified until that command and an install/launch smoke have both succeeded in the current environment. Host Rust/Vitest tests do not exercise Android JNI registration, Keystore, FileProvider sharing, WebView behavior, permissions, or process/lifecycle recovery; those require an emulator or physical-device test.

**Current Task 18 host evidence (2026-09-02):** frontend tests are 832/832, the default desktop Rust library is 1,184/1,184, the MCP-only binary is 958/958, the protocol suite is 21/21, the feature-gated E2E runtime is 19/19, and the exact ARM64 Kotlin contract suite is 11/11. Fresh Playwright desktop and Pixel journeys passed 2/2 with strict browser-console/page-error gates empty. The desktop Tauri release and NSIS bundle passed. The canonical Android wrapper rebuilt the frontend and ARM64 Rust library, then reached the documented Windows host boundary because Developer Mode is unavailable for its final native-library symlink; `HOW_TO_RUN.md` records the reviewed copy-and-clean-assemble fallback. That fallback produced a v2-signed 392,893,783-byte debug APK with SHA-256 `7DF91851746501BF14F3F45D743EA0F60EF59B2905C2207D6F5C3E07F915AA3B`; its 385,622,328-byte packaged native library exactly matches the current source SHA-256 `7E71D9C32A7BC8B5DEF7B918A57A11413ADDCAEED279FF21F90CB8AFF59A38D5`. No `adb` device is attached, so install/launch and real Keystore, FileProvider, WebView, permissions, and lifecycle behavior remain unverified.
...
# Real-system desktop + Pixel browser journeys (after npm ci in both trees)
cd ../e2e
...
On Windows/MSVC, `build.rs` passes the Common Controls v6 manifest dependency to Rust test harnesses. Tauri's generated resource already owns the application manifest for both binary targets that link it, so `grafyn` and the feature-gated `grafyn-test-runtime` use `/MANIFEST:NO` to avoid duplicate MANIFEST resources while Rust test executables retain the activation dependency. This prevents the `TaskDialogIndirect` loader failure; if a stale `target/debug/deps/grafyn_lib-*.exe` still shows that popup, rebuild the harness rather than launching the stale artifact. The Rust suite is filesystem-heavy, so use `cargo test -- --test-threads=4` on Windows to avoid transient scanner/ACL pressure from unrestricted parallel temp-root churn.

## Architecture Overview

**Current implementation:** Grafyn uses one Tauri 2 Rust library entry point with separate desktop and Android command registrations. Desktop keeps the existing wide shell, user-selected vault, spatial/linear Canvas, Ollama/MCP/import/migration/optimizer/updater surfaces, and one-shot image generation. Android mounts the compact companion shell over app-private storage and exposes the bounded local Capture, Recall, Twin review/chat, and chronological one-model Canvas surfaces, plus a dormant native image-share command boundary. Backend `RuntimeStatusV1`, loaded before router mount, is authoritative: the frontend's candidate profile can only remove capabilities, never invent them. Android image generation remains false; native Share is separately health-gated and also remains false because production Android has no image-receipt producer or import path. No web mode, Python backend, hosted relay, billing, account, or pairing/recovery service exists.

### Companion-first direction (owner decision, 2026-08-29; Android runtime boundary implemented in Task 17)

Grafyn's mission is the local-first data-collection and governed-state layer for a person's digital twin. Desktop and Android are peer local clients, with a user-selected desktop vault and an app-private Android vault. Models may propose patterns, but explicit review/verification governs durable Twin memory; privacy and allowed-use gates run before per-use-case attention ranking. Time, relationship, activity, environment, and goal context remain first-class rather than being collapsed into one global trait or importance score.
...
- MPL-2.0 for the open client/local core, with the provenance decision recorded in `RELICENSING.md`; the existing `frontend/src-tauri/crates/grafyn-sync-protocol` package uses `Apache-2.0 OR MPL-2.0`, while any future hosted service remains separate and unavailable until built.
...
│  Vue WebView → target-specific IPC allowlist      │
│                                                    │
│  Desktop wide             Android compact          │
│  user-selected vault      app-private vault        │
│  wide + companion UI      companion UI only        │
│  desktop keychain         Android Keystore bridge  │
│                                                    │
│  Canonical local events + deterministic projection │
└──────────────────────────────────────────────────┘
```

Android roots are conceptual, never public IPC values: app data contains `Grafyn/config`, `Grafyn/data`, and `Grafyn/vault`; app cache contains `Grafyn/`, with FileProvider staging restricted to `Grafyn/grafyn-share-v1`. Cloud-backup and device-transfer extraction rules explicitly exclude every app-private data domain on current Android, with matching legacy full-backup exclusions. The settings/status DTOs redact the app-private vault path. Desktop retains its existing user-selected vault and normal OS-specific config/data/cache locations.

Android constructs no Ollama, link-discovery worker, Markdown-migration, vault-optimizer, MCP, updater/process, native vault picker, or arbitrary-path import service. If canonical event/root recovery fails, startup returns a recoverable failed `BootStatus`, disables canonical capabilities, and never exits the process or serves a disposable/default Twin. Missing native registration, or an unavailable Keystore with no durable secret records, keeps the canonical local `MutationCoordinator` active for offline notes, Capture, Recall, and Twin review but does not construct `SyncEngine`; OpenRouter/Twin chat and other secret-dependent capabilities fail closed. Each native secret record stores a non-secret canonical account mapping beside its nonce, ciphertext, and tag; the account hash must bind to the record prefix, and health loads the existing Keystore key once and AEAD-authenticates every durable record with that exact account as associated data. Corrupt mappings, ciphertext, tags, missing durable-record keys, or malformed/unknown health replies from a registered native backend enter recoverable failed boot with the fixed `android_native_health_invalid` status. Android neither reads environment secrets nor migrates plaintext OpenRouter keys: finding a plaintext key in Android settings fails startup before native secret-store access and leaves the file untouched.

`get_runtime_status` returns bounded, path-free `RuntimeStatusV1`. On Android, canonical health gates notes read/write, Recall, Twin review, and linear Canvas; Twin chat additionally requires secure-secret health; image generation is always false; native image share additionally requires native share health; and sync additionally requires secure-secret health plus a constructed `SyncEngine`. Spatial Canvas, native vault picking, path import, local Ollama, MCP, vault migration, optimizer admin, and desktop updater are always false. Recoverable capability health is represented as sanitized `ready | unavailable`; corrupt or invalid registered-native health is represented only by the fixed failed-boot status, never raw errors or paths.

The Android invoke handler is deliberately compact: runtime/boot status; note CRUD; compact Canvas session/model/send/regenerate operations; required Twin workspace reads, proposal/digest review, and Canvas feedback; governed capture/projection/attention/timeline; image receipt save/load/discard and native share; redacted settings/OpenRouter status; sync status; and Recall. Search/retrieval administration, debate, spatial Canvas, arbitrary-path export/import, sync export/import/rebuild, Ollama, MCP, migration, optimizer, updater, and process commands are not registered.
...
- `e2e/` is a committed two-journey Playwright system suite wired into the `system-e2e` CI job. It starts Vite, a feature-gated local Rust runtime, and a deterministic OpenRouter-compatible fixture; notes, attachments, Twin events/projections, restart, and encrypted two-device sync stay on production Rust services over per-run temporary roots. The Pixel profile injects an in-memory test secret store and enables image generation only in that harness. A Canvas terminal-wait failure permanently quarantines that harness device for the run instead of rebuilding over a root that a detached task may still mutate. Production Android still reports image generation unavailable, and browser E2E is not installed-APK, Keystore, FileProvider, WebView, or lifecycle evidence.

### Tauri IPC Commands

The normal desktop shell registers command modules from `frontend/src-tauri/src/commands/`. `canvas` is a directory module (split — see below); every other normal module is a single file. The `mcp` command module and both MCP invoke registrations are compiled only on desktop targets. Enumerate exact command names with `grep -rn "#\[tauri::command\]" -A1 src/commands/` — purposes only below, to avoid drift.
...
| `image_generation.rs` | Live image-model discovery/capabilities, one-shot non-streaming generation receipts, governed local save/load, desktop Save As, and Android native sharing of an existing validated receipt; Android generation remains unavailable |
...
| `evidence.rs` | Desktop-only evidence workspace: snapshot, interview, goals, relationship review, local embedding setup, sealed prediction capture |
...
| `twin_state.rs` | Governed observation/proposal pages, explicit review, deterministic temporal projection/timeline, per-profile attention traces, and transactional companion capture |
...
- `src/platform/runtime.js` / `src/platform/capabilities.js` — explicit `desktop-wide`, `android-compact`, and `plain-web` profiles. Android and iOS are compact; every other Tauri platform is wide desktop. Before the router mounts, Tauri loads the typed backend `RuntimeStatusV1`; that status is authoritative, and the frontend profile can only further restrict it. The capability matrix includes notes, Twin, Canvas, image generation, native image share, sync, native-vault, import, Ollama, MCP, migration, optimizer, and updater access; `assertCapability()` fails closed with `CapabilityUnavailableError`. Android image generation stays false, while native image share is reported independently from bridge health.
- `src/api/client.js` — preserves one namespace per command module plus `images`, `optimizer`, `isTauriApp`, and `isDesktopApp`, delegating each call through the injectable transport in `src/api/transport.js`. Its only public runtime bridge operations are `invoke`, `listen`, `openExternal`, and `showMainWindow`; `src/api/tauriTransport.js` is the static Tauri import boundary. In Vite development only, exact loopback URL/token variables may select the bounded authenticated `e2eTransport`; its desktop/Android query selector is frozen when the module activates and its profile always sets `nativePlugins: false`. Production builds select the Tauri transport and tree-shake the test bridge. Updater/process plugins remain deliberately desktop-only dynamic imports in `App.vue`.
- `src/composables/useCompanionLayout.js` selects compact presentation for Android profiles or viewports at most 767px. Compact `App.vue` mounts `CompanionShell` with four persistent destinations, `100dvh`, safe-area insets, visible focus, and non-hover actions. Android exposes only capabilities confirmed by the runtime status; a narrow desktop uses the enabled desktop services through the same responsive surface.
- `src/composables/useRecallSearch.js` is the single debounced recall adapter used by both compact Recall and the existing desktop `SearchBar`. It normalizes Rust response shapes, suppresses stale responses, clears stale results while a new query is pending, and exposes safe local error states.
- **Pinia stores (5):** `canvas.js`, `theme.js`, `boot.js`, `twin.js`, `evidence.js` — Twin records, Constitution, action gaps, decisions, Decision Mirror config, governed observation/proposal pages, projection, timeline, attention traces, and proposal-review refresh state live in the Twin store; the evidence store owns interview/goals/relationships/prediction workspace state. Components read/act through these stores rather than holding local copies. Twin resource/workspace loaders use per-resource latest-generation fences, and compact-view unmount invalidates pending commits and stale review refresh continuations. Canvas session loads and creates use the same fail-closed ownership generation, while per-response feedback is deduplicated until its exact request finishes. `canvas.sendCompanionPrompt()` is the compact one-model seam over the existing persisted/streamed send path; it maps plain/Knowledge/Twin modes and accepts only Advisor or Twin-only Simulation answer modes plus the exact persisted Twin relationship variant.
- `components/companion/` now includes the compact Twin review queue/chat, linear Canvas session/thread/composer, and `QuickImageComposer` surfaces. Review actions preserve exact proposal/digest IDs; one reference time is shared across each Twin refresh; stale attention explanations and review actions stay hidden/locked until the matching refresh completes. Relationship selectors preserve unique normalized full variants (including multi-edge variants), and selector/chat/turn/review/memory/timeline labels expose relationship direction. Twin chat freezes and persists the submitted relationship context, shows only matching turns, automatically parents only the latest matching completed usable response, labels Global versus exact relationship context, and excludes tile/evidence context mismatches. The `twinChat` capability independently controls Chat without removing governed review. Twin chat sessions carry `companion-twin-chat` and are never adopted or listed by compact or wide Canvas; tagged deep links are cleared and redirected to Canvas root. Twin chat reopens only the newest tagged history and fails closed if discovery cannot prove ownership. Linear Canvas sorts sessions by `updated_at` descending then ID, sorts turns by `created_at` ascending then ID, keeps follow-up, regenerate, and accept/reject controls visible, and owns drafts, duplicate-send locks/completion, action errors, and store error display by session so a late result cannot overwrite or block another thread while its replacement route is still loading; an unchanged draft clears only after its exact send succeeds. Wide Canvas opens the same image workflow in `ImageGenerationDialog`; compact Canvas mounts it without importing D3, and neither path touches Canvas text-model selection or streaming state.
- `components/twin/` — the Twin Workspace's tab components (`TwinOverviewTab.vue`, `TwinSetupTab.vue`, `TwinConfigTab.vue`, `TwinConstitutionTab.vue`, `TwinActionGapsTab.vue`, `TwinDecisionsTab.vue`, `TwinMemoryTab.vue`, `TwinGuideTab.vue`, plus desktop Evidence Workspace tabs) plus shared pieces (`ReviewActions.vue`, `SetupField.vue`, `ActionGapRow.vue`, `DecisionRow.vue`, `EvidenceDrawer.vue`) and `twin-workspace.css`. `views/TwinReviewView.vue` is a thin shell that wires the Twin and evidence stores to these tabs — do not add Twin Workspace logic directly to the view.
- **Routes:** `/` uses `ResponsiveHomeView` (wide notes Home or compact Capture); `/twin` uses `ResponsiveTwinView` (wide `TwinReviewView` workspace or compact governed review/chat/memory); `/canvas` and `/canvas/:id` use `ResponsiveCanvasView` (wide D3 spatial Canvas or compact linear Canvas); `/recall`, `/import`, and catch-all retain their existing views. The Canvas route and bottom-nav gate is `linearCanvas`, not `spatialCanvas`, so both a narrow desktop and the Android compact runtime can enter the responsive route when the backend advertises it; async responsive branches prevent compact Canvas from loading or mounting D3.
...
Capture records durable evidence rather than UI execution noise: notes emit `NoteChanged`; each conversation/document import adds one empty-claim container observation; companion text capture atomically adds its contextual empty-claim observation beside the inbox note; persisted Canvas prompts, completed responses, regeneration, debate turns/responses, and explicit feedback use their dedicated payloads; and legacy Twin text plus generic Canvas feedback/insights remain empty-claim observation evidence. The sole Canvas-feedback claim path is an explicit user-written `Insight + Preference` tied to a completed persisted global response. The capture dialog freezes that response's durable ID and exact content when it opens, keeps regeneration locked for that response while the local capture is open or committing, and sends the frozen witness; under the shared coordinator lock, the backend requires it to match both the fresh persisted response and one exact active `CanvasResponseRecorded` event. A stale, missing, or ambiguous witness fails closed. The resulting observation inherits that response event's governance, names the exact response event as evidence and causal parent, and prevents the same logical response from counting twice toward the same claim/proposal. It remains an observation: three matching captures from three distinct responses derive only a pending proposal, which still requires human review before Advisor/Simulation use. Relationship-scoped preference capture fails closed until full relationship provenance can be preserved. Canvas responses persist bounded provider/provenance at generation time, and events copy those values without model-name inference. An explicitly saved generated image commits its content-addressed attachment, metadata note, and one empty-claim observation through the same governed mutation boundary; generation/preview alone emits nothing. A decision submission persists its Canvas tile, decision episode, trace, primitive assessment, and single `DecisionRecorded` group atomically. Once its sealed prediction request is durable, every same-root abandonment attempts an idempotent requested-to-failed transition against current exact authority; a genuine root/lease switch touches neither vault, and an already committed visible response is preserved. Generic legacy record writes cannot change promotion/privacy/rejection/training state; explicit governance actions use the feedback boundary. Derived indexes, optimizer overlays, digests, parser sections/turns, pending or failed Canvas states, layout/viewport/structural edits, and general product diagnostics emit nothing. A local/private/restricted member makes its whole event group local-only. Native `MemoryProposed`/`MemoryReviewed` is created only by the explicit proposal-review boundary; the sync operation/attachment foundation, companion text capture, and governed one-shot image capture exist, while hosted relay and video capture remain unimplemented.
...
### One-Shot Governed Image Generation

`commands/image_generation.rs` and `services/openrouter/image_generation.rs` provide a non-streaming image path that is deliberately separate from Canvas text sessions, stores, model selection, and streaming. The frontend discovers the live image catalog, fetches the selected model's current endpoint capabilities, and requires an explicit model, square-capable resolution, and `1:1` aspect before one request; compact Canvas delays discovery until the user expands `QuickImageComposer`. Grafyn sends the exact visible multiline prompt once with no automatic retry. Generation returns an expiring in-memory receipt plus bounded PNG/JPEG/WebP bytes and an exact-USD or unavailable cost; it does not mutate the vault.

The preview uses a MIME-typed Blob URL, and the owner revokes it on confirmed discard or unmount; Tauri CSP admits `blob:` only under `img-src`. Explicit discard, an abandoned mounted preview, and a generation that resolves after its owner unmounts also invoke the idempotent backend receipt-discard boundary so prompt/image bytes do not linger until expiry; local cleanup never depends on that best-effort IPC succeeding. While a receipt exists, the prompt/model/resolution/aspect provenance is frozen, but the optional save annotation and explicit `strip_metadata | retain_original` and `local_only | inherit` policies remain editable. `save_generated_image` consumes the receipt and atomically commits the app-owned content-addressed attachment, metadata note, observation event, and attachment sync work. Its response reports this attachment's exact `local_only | awaiting_provisioning | queued` disposition plus manifest/chunk/operation counts; the UI never presents global outbox totals as this image's state. A committed/recovery-pending `do not retry` response and an unavailable receipt both lock further receipt actions. Desktop `export_generated_image` is a distinct Save As action that receives only receipt ID plus retention policy and never an arbitrary path. Android generation and native sharing remain capability-disabled because Android has no production image-receipt producer or import path. The validated native command remains dormant: it can stage MIME-matched bytes only below the app-cache FileProvider share root and may report only that the share sheet opened; it does not claim recipient delivery or observe chooser cancellation.

The `e2e-test-runtime` Android profile is deliberately different: after constructing the production runtime over temporary owned roots with an injected `MemorySecretStore`, the harness alone advertises image generation so Playwright can exercise the portable generation/save core. This does not change `RuntimeBootstrap`, the installed Android capability matrix, or the native verification boundary.
...
Untrusted envelopes must enter through the raw-size-bounded `EnvelopeV1::from_json`/`from_json_bytes` boundary; generic Serde deserialization is intentionally unavailable. JSON fields and canonical Base64URL spellings are strict, secret wrappers zeroize and redact `Debug`, and plaintext-bearing types expose metadata/length-only debug output. The normative schema, golden vector, interoperability rules, visible relay metadata, and threat model live in `docs/sync/`. The client now includes the public protocol, stable vault/device identity, secure-secret boundary, and a local manual ciphertext sync engine with deterministic convergence and recovery. Production transport/relay, pairing/recovery/revocation, accounts, billing, acknowledgement/pruning, and hosted operations do not exist and must not be represented as connected or available.
...
7. Candidate Constitution Hypotheses (when present)
...
| Variable | Notes |
|----------|-------|
| `OPENROUTER_API_KEY` | Required for Multi-LLM Canvas (including note context), one-shot image generation, distillation, link discovery |
...
`.github/workflows/test.yml` — runs on push to main and PRs. Jobs: `release-preflight` (version + Cargo.lock alignment), `rust-tests` (ubuntu-22.04), `frontend-tests` (Vitest), `system-e2e` (real Rust services plus deterministic OpenRouter fixture, desktop-wide and Pixel browser journeys), `lint` (eslint + file-size tripwire + separate default-desktop, MCP-binary, and E2E-runtime `cargo clippy -D warnings` profiles), `security` (RustSec plus both npm audits), `build` (Vite), `android-build` (secret-free arm64 debug APK plus native contracts), and `test-summary`. RustSec vulnerabilities and npm audit findings at high severity or above block PRs; each supported Clippy profile currently runs with `continue-on-error: true` pending cleanup of pre-existing warnings (see the TODOs in the `lint` job). Grafyn does not use an all-target/all-feature union because that would compile the standalone MCP binary with Tauri-only cfg branches, an unsupported product combination. The file-size tripwire (`npm run check:file-sizes`, `scripts/check-file-sizes.cjs`) blocks PRs when any non-test `.rs`/`.vue`/`.js` source file exceeds 2,500 lines.
...
`tauri.conf.json` is Android-neutral. Desktop builds must merge `tauri.desktop.conf.json` so the MCP external binary, updater endpoint/public key, `createUpdaterArtifacts: "v1Compatible"`, and desktop capability are present. The main desktop window keeps `useHttpsScheme: true`; changing it would reset the legacy HTTPS webview origin's IndexedDB, localStorage, and cookies for existing users.
...
**Data location:** Desktop uses the user-selected vault plus OS app config/data/cache directories. Android keeps config, canonical data, and its vault in app-private storage, with transient image-share files under the app cache's dedicated FileProvider subtree. Runtime status exposes only the vault kind and availability, never filesystem paths.
