...
- **Routes:** `/` uses `ResponsiveHomeView` (wide notes Home or compact Capture), `/recall` is compact full-screen Recall, `/canvas`, `/canvas/:id`, `/import`, `/twin` (component: `TwinReviewView.vue`, thin shell over `components/twin/`), plus catch-all → `NotFoundView.vue`. Capability guards redirect unsupported runtime routes to a local unavailable screen; Android path import fails closed.
