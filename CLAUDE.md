...
Environment: `set OPENROUTER_API_KEY=your-key` (Windows) or `export OPENROUTER_API_KEY=your-key` for Canvas text and one-shot image generation
...
**Current implementation:** Grafyn uses a Tauri 2 shared Rust library entry point with a working wide desktop shell and Vue frontend. At narrow desktop widths, the adaptive companion shell provides governed Capture, Recall, Twin review/chat, chronological one-model Canvas, and one-shot image generation without loading the wide Home, Twin workspace, or D3 Canvas; those wide desktop views remain unchanged. Desktop enables linear/spatial Canvas and image generation, while Android initialization, app-owned storage, secure OpenRouter secrets, image generation/share, and other mobile capability enablement remain unavailable until Task 17 lands. No web mode and no Python backend exist.
...
│  │   migration, image generation, boot)        │  │
│  └──────────────────────────────────────────┘  │
│  ~/Documents/Grafyn/                          │
...
| `image_generation.rs` | Live image-model discovery/capabilities, one-shot non-streaming generation receipts, governed local save/load, and desktop Save As; mobile generation/share remains fail-closed until Task 17 |
...
- `src/composables/useCompanionLayout.js` selects compact presentation for Android profiles or viewports at most 767px. Compact `App.vue` mounts `CompanionShell` with four persistent destinations, `100dvh`, safe-area insets, visible focus, and non-hover actions. Android capabilities still fail closed until Task 17; a narrow desktop uses the enabled desktop services through the same responsive surface.
...
- **Routes:** `/` uses `ResponsiveHomeView` (wide notes Home or compact Capture); `/twin` uses `ResponsiveTwinView` (wide `TwinReviewView` workspace or compact governed review/chat/memory); `/canvas` and `/canvas/:id` use `ResponsiveCanvasView` (wide D3 spatial Canvas or compact linear Canvas); `/recall`, `/import`, and catch-all retain their existing views. The Canvas route and bottom-nav gate is `linearCanvas`, not `spatialCanvas`, so a narrow desktop can enter the responsive route while Android remains unavailable until Task 17; async responsive branches prevent compact Canvas from loading or mounting D3.
...
The preview uses a MIME-typed Blob URL, and the owner revokes it on confirmed discard or unmount; Tauri CSP admits `blob:` only under `img-src`. Explicit discard, an abandoned mounted preview, and a generation that resolves after its owner unmounts also invoke the idempotent backend receipt-discard boundary so prompt/image bytes do not linger until expiry; local cleanup never depends on that best-effort IPC succeeding. While a receipt exists, the prompt/model/resolution/aspect provenance is frozen, but the optional save annotation and explicit `strip_metadata | retain_original` and `local_only | inherit` policies remain editable. `save_generated_image` consumes the receipt and atomically commits the app-owned content-addressed attachment, metadata note, observation event, and attachment sync work. Its response reports this attachment's exact `local_only | awaiting_provisioning | queued` disposition plus manifest/chunk/operation counts; the UI never presents global outbox totals as this image's state. A committed/recovery-pending `do not retry` response and an unavailable receipt both lock further receipt actions. Desktop `export_generated_image` is a distinct Save As action that receives only receipt ID plus retention policy and never an arbitrary path; Android generation and native Share remain capability-disabled until Task 17's secure secret/native boundary.
