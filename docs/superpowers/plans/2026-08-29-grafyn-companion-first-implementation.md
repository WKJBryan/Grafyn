...
- [x] Write failing tests for review accept/reject/edit, attention explanation, relationship filter, history-aware chat parent IDs, Simulation gate/disclosure, and evidence snapshot display.
- [x] Write failing Canvas tests for wide/compact selection, no `CanvasContainer` mount in compact mode, chronological ordering, session CRUD, follow-up context, streaming/error visibility, regenerate, and accept/reject actions without hover.
...
Task 15 backend verification passed 146/146 focused Rust tests across Twin History, Canvas context/streaming/models/store, projection, and Twin-state model/command suites. Projection v3 preserves the proposal's exact relationship context across review, conflict, and supersession timeline transitions, and non-empty state filters require full-variant equality. Timeline filtering uses each temporal entry's exact relationship variant as authority while applying goals/tags independently, retaining valid supersession transitions whose current item is gone and source event belongs to another context. New global prompts stamp `global-v4` and select global memory only; relationship prompts stamp `relationship-v4` and select global plus the exact whole variant. Missing version metadata is accepted only for byte-compatible legacy global ctx-v3 replay, whose historical all-context selection and exact projection-v2 evidence remain usable only while projection and prompt evidence reproduce exactly; global-v4 rejects projection-v2 IDs. Central evidence validation makes Canvas persistence/load, replay, and parent-chain reconstruction reject every cross-relationship ancestor, pending/errored/empty response, and forged or unsupported version/context combination.
...
- [x] Write failing request mapping, API-key/capability, provider-error, empty-image, MIME, base64, decoded-size, and path-safety tests.
- [x] Add separate non-streaming model-discovery and `images.generate({ prompt, modelId, resolution, aspectRatio })` commands. Send exactly one image and `stream: false`; require explicit live-discovered model selection; enforce advertised resolution/aspect capability before spending; use a dedicated approximately 180-second timeout with no automatic retry. Cap JSON at 40 MiB, decoded image at 24 MiB, either dimension at 4096, and total pixels at 16,777,216. Accept only fully decoded PNG/JPEG/WebP with matching MIME, and reject SVG/unknown/mismatched media. Do not route image bytes through Canvas text response persistence.
- [x] Write frontend tests for loading, duplicate-submit prevention, Blob URL creation/revocation, typed errors, exact-or-unavailable cost, discard warning, desktop Save As, Android file share, explicit governed save, and offline/unconfigured states. Wide and compact Canvas share the feature without changing text-model state.
- [x] On save, use the canonical note/event mutation path and content-addressed app-owned attachment storage with digest, MIME, byte size, dimensions, source, annotation, and EXIF-retention policy; thumbnails are derived. Expose no arbitrary filesystem path on Android and surface Task 12's real attachment outbox/status instead of a generic success claim.
- [x] Run focused/full tests and inspect a saved fixture.
- [x] Commit: `feat: add governed companion image generation`

**Frontend verification (2026-09-01):** The Task 16 affected frontend suite passed 238/238 tests across 15 files and the full Vue suite passed 725/725 tests across 64 files. The production Vite build completed after transforming 380 modules, and selected ESLint passed with zero errors or warnings. RED regressions cover live/lazy discovery, exact supported endpoint pairs, exact multiline prompts, duplicate generation locks, typed offline/network/unconfigured/provider and terminal receipt errors, exact-or-unavailable cost, Blob URL creation/replacement/unmount revocation, idempotent backend receipt discard after explicit discard/unmount/late completion, provenance-field freezing, attachment-specific local-only/awaiting/queued sync disposition, terminal save/export locks, retention-aware desktop Save As without arbitrary paths, Android Share fail-closed capability UX, unchanged Canvas text-model state, compact no-D3 integration, and nested modal ARIA/focus placement/Tab/Shift-Tab/Escape/restoration. The frontend made no paid/provider request during verification.

**Backend verification (2026-09-01):** The authoritative Rust library suite passed 1,121/1,121 tests. Focused suites passed 39/39 image-generation tests, 52/52 sync-engine tests, and 8/8 atomic-I/O tests; independent release checks reran five critical post-commit, raster-provenance, projection-collision, and Windows-retry seams serially at 1/1 each. The saved local-only fixture proves one note, two governed events, exact attachment bytes and metadata, a derived thumbnail, one-shot receipt consumption, and zero sync work; desktop retention fixtures prove byte-exact retain-original export and metadata removal for strip-metadata. `cargo fmt --check`, the 2,500-line source-size gate, and `git diff --check` passed. The broader `clippy -D warnings` gate remains blocked by pre-existing repository-wide lint debt (92 test-target findings); the Task 16 warning in `secure_fs` was resolved. No paid OpenRouter request was made, and a fresh Windows audit after the full suite found no `TaskDialogIndirect`/entry-point event, stale executable, or lingering Rust test process.
...
- [x] Probe Java, Android SDK/NDK, Rust Android targets, and `adb`; record exact missing prerequisites before installing anything.
- [x] Run `npx tauri android init`; review generated identifiers/minimum SDK before accepting generated files.
- [x] Write failing Rust/frontend capability and path tests: Android app-private directories; no Documents assumption; no MCP external binary; no updater/Ollama/path picker/import/migration/optimizer; local notes/Twin/Canvas/image APIs available as declared.
- [x] Implement platform-specific service registration. Desktop services remain intact; Android does not even initialize unavailable dependencies.
- [x] Make canonical vault/event/attachment initialization fail into a recoverable boot screen; only search, graph, chunk, thumbnail, and projection caches may be recreated automatically.
- [x] Delete a reviewed-memory projection cache in a test, restart, and prove it rebuilds identically from accepted review events; corrupt/missing canonical events must fail closed instead of falling back to a stale cache.
- [x] Implement one secure Android secret adapter used by both sync and OpenRouter through the shared secret abstraction, using an OS-backed mechanism supported by the selected Tauri plugin/native layer. Target-gate `keyring`, `notify`, PDF/DOCX tooling, MCP, updater, and other desktop-only dependencies. Add a release-build assertion that rejects plaintext/fallback storage. If no production-grade adapter can be verified on this host, every secret-dependent Android capability (sync, OpenRouter text, and image generation) remains disabled and the UI/build diagnostics must say so explicitly.
- [x] Add compact settings for theme, local vault summary, OpenRouter secret entry/status, sync state, and capability diagnostics. Desktop-only settings never render or invoke.
- [x] Add Android CI compile/build steps with documented secret-free debug behavior.
- [x] Run `npx tauri android build --debug` and, if an emulator/device is available, install/launch and capture logs. Otherwise run the mobile-compatible Rust target check and record the precise external boundary.
- [x] Update live docs with commands, data paths, capability matrix, and mobile limitations.
- [x] Commit: `feat: initialize secure Android Grafyn companion`

**Verification (2026-09-01):** The full frontend suite passed 768/768 tests across 67 files; ESLint completed with zero errors and seven known pre-existing warnings; Vite built 382 modules; and the 2,500-line source-size gate passed. The desktop Rust library passed 1,161/1,161 tests and the MCP-only binary passed 941/941; `cargo fmt --check` and `git diff --check` passed. The exact ARM64 Kotlin contract suite passed 11/11 tests, including authenticated health rejection of same-length ciphertext and tag tampering, and both `cargo check --locked --lib --target aarch64-linux-android` and a fresh Android native build completed. Mounted Recall, Canvas, and Twin-review routes now reactively revoke stale authority; the RED suite failed five intended cases before the focused 32/32 and adjacent 106/106 GREEN passes. Ubuntu CI invokes the generated Gradle wrapper through `bash ./gradlew`, making the Linux job independent of executable mode recorded by a Windows checkout. The canonical Tauri wrapper rebuilt the frontend and native library but reached the documented Windows host boundary when Developer Mode was unavailable for its final native-library symlink. Copying that exact built library into generated `jniLibs` and running a clean `assembleArm64Debug` produced a 392,359,999-byte APK with SHA-256 `F78B05C99C3053CE52D7B8FCBEB88B0831648094A7B5F781AC72AC65CABA56C2`; APK Signature Scheme v2 verification passed with one Android debug signer. The packaged native library was 385,088,544 bytes with SHA-256 `D184078D729A921A916A6E362272BE96D43FE5199947E4A212A7CB7585691499`, exactly matching the source build. `adb devices -l` reported no emulator or device, so install/launch, real Keystore/FileProvider/WebView, and lifecycle proof remain an explicit external boundary rather than a claimed pass. A final Windows audit found the stale popup executable absent, the replacement harness present, zero matching processes, and no matching `TaskDialogIndirect`, Application Popup/Error, WER, or SideBySide event.
