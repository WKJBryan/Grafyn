<template>
  <div class="unlinked-mentions">
    <PanelHeader
      ...
      :count="mentions.length > 0 ? mentions.length : null"
      :collapsible="true"
      :expanded="isExpanded"
...
async function fetchUnlinkedMentions() {
  ...
  loading.value = true
  try {
    mentions.value = (await graph.unlinkedMentions(props.noteId)) || []
  } catch (error) {
...
watch(isExpanded, (expanded) => {
  if (expanded && mentions.value.length === 0) fetchUnlinkedMentions()
})

watch(() => props.noteId, () => {
  mentions.value = []
  if (isExpanded.value) fetchUnlinkedMentions()
})
