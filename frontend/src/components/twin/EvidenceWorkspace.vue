  ...
  <section class="tab-panel evidence-workspace">
    <header class="evidence-section">
      ...
      <div class="evidence-actions">
        ...
        >
          Set up / open pilot</button
        ><button class="btn btn-secondary" :disabled="store.busy" @click="store.load">
          Refresh</button
        ><button
      ...
      <p v-if="store.installingEmbeddings" role="status">
        Downloading the embedding model into local Ollama. This can take several minutes.
      </p>
      <p v-if="store.busy" role="status">Working…</p>
      ...
      <details v-if="store.snapshot?.jobs.length">
        ...
        <ul>
          <li v-for="job in store.snapshot.jobs" :key="job.id">
    ...
    <template v-if="store.snapshot && (targetConfigured || tab === 'relationships')">
      <EvidenceInterview v-if="tab === 'interview'" :key="store.snapshot.subject_id" />
    ...
    <p v-else-if="store.scope === 'current' && !store.busy">
      ...
      <router-link to="/import">Import and map a target person</router-link>
    </p>
