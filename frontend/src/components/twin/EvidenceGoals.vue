  ...
  <section class="evidence-section">
    ...
    <ul class="evidence-list">
      ...
      >
        ...
        <p>{{ goal.definition }}</p>
        <span
          v-for="criterion in goal.criteria"
          :key="criterion.id"
    ...
    >
      <fieldset :disabled="store.busy">
        <label>Goal name<input
          v-model="draft.label"
          required
        ></label>
        <label>What does success mean in your words?<textarea
        ...
        <label>Status<select v-model="draft.status">
          <option
            v-for="state in ['active', 'paused', 'achieved', 'abandoned']"
            :key="state"
          >
            {{ state }}
          </option>
        </select></label>
        <label>Context-specific priority<textarea
          v-model="draft.contextual_priority"
          rows="2"
        />
        </label>
        <label>Constraints and unacceptable costs — one per line<textarea
          v-model="constraints"
          rows="2"
        />
        </label>
        <fieldset>
          ...
          <label
            ...
            class="inline-label"
          ><input
             v-model="draft.competing_goal_ids"
             type="checkbox"
             :value="goal.id"
           >
            {{ goal.label }}</label>
        ...
        >
          ...
          >{{ field.label
            ...
            v-model="criterion[field.key]"
            placeholder="Unknown / not supplied"
          ></label>
          <label
          ...
          <label class="inline-label"><input
            v-model="criterion.is_proxy"
            type="checkbox"
          > This is an agreed proxy</label>
          <button
            class="btn btn-secondary"
            type="button"
            @click="draft.criteria.splice(index, 1)"
          >
        ...
        <label>When did this change take effect? Optional date/time with timezone<input
          v-model="draft.effective_at"
          type="text"
          placeholder="For example: 2026-09-05T12:00:00+08:00"
        ></label>
        <label>Reason for this revision<textarea
    ...
    <details v-if="store.snapshot.goals.length">
      ...
      <ul>
        <li
          v-for="goal in store.snapshot.goals"
